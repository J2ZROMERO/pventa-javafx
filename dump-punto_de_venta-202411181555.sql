...
INSERT INTO `inventories` VALUES (39,'r23','2024-11-05 00:00:00',0,0,'2024-11-21 00:00:00',''),(40,'780083146856',NULL,50,5,NULL,'');
...
INSERT INTO `products` VALUES (5,'gansito','aaaaaa','dsadsad',0,123,321,'dsad','asdsad',10),(6,'refresco','asdadsad','r23',0,550,50,'bebida','pepsi',11),(7,'sol','','s23',0,42,NULL,'bebidas','cuahutemoc',NULL),(9,'sol','mangochada','7503032272375',0,15,NULL,'','',NULL),(10,'uniquin','pastilla para diarrea','780083146856',0,45,NULL,'farmacia','',10);
...
INSERT INTO `sells` VALUES (1,6,1,550,550,'2024-11-18 12:32:32','pieza'),(2,10,5,45,225,'2024-11-18 12:48:17','pieza'),(3,9,3,15,45,'2024-11-18 12:48:17','pieza'),(4,10,5,45,225,'2024-11-18 13:59:07','pieza'),(5,9,4,15,60,'2024-11-18 13:59:07','pieza'),(8,10,15,10,152,'2024-11-18 14:22:00','test'),(9,10,15,10,152,'2024-11-18 14:22:32','test');
...
INSERT INTO `suppliers` VALUES (9,'fdsfsdfsdf','','',''),(10,'nuevo arrengo','asdasd','asdasd','dasd'),(11,'otri nuevo','','','hhbvcbvcbcvbbcbvcb'),(12,'gfdgdgd','435435','gfgfdgdf','gfdgfdgfdgfdgf');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddSale`(
    IN p_id_product VARCHAR(100),  -- Product Code
    IN p_sold_amount DOUBLE,       -- Amount Sold
    IN p_unit_price DOUBLE,        -- Unit Price
    IN p_total_sold DOUBLE,        -- Total Sold
    IN p_sell_type VARCHAR(100)    -- Sale Type
)
BEGIN
    DECLARE available_stock DOUBLE;
    DECLARE product_id BIGINT;

    -- Step 1: Retrieve the product ID from the products table
    SELECT id
    INTO product_id
    FROM products
    WHERE code = p_id_product;
    ...
    -- Step 3: Check the current stock in inventories
    SELECT amount_available
    INTO available_stock
    FROM inventories
    WHERE fk_code = p_id_product;

    -- Step 4: Verify if there is enough stock
    IF available_stock >= p_sold_amount THEN
        -- Step 5: Subtract the sold amount from available stock
        UPDATE inventories
        SET amount_available = amount_available - p_sold_amount
        WHERE fk_code = p_id_product;

        -- Step 6: Insert the sale into the sells table
        INSERT INTO sells (fk_product, sold_amount, unit_price, total_sold, sell_type)
        VALUES (product_id, p_sold_amount, p_unit_price, p_total_sold, p_sell_type);
    ELSE
        -- Step 7: Raise an error if there isn't enough stock
        SIGNAL SQLSTATE '45000'
...
BEGIN
    SELECT 
        ...
        p.unit_price,
        p.volume_price,
        COALESCE(SUM(i.amount_available), 0) AS stock,  -- Return in-stock value
        p.category,
        p.brand,
        p.fk_supplier
    FROM 
...
BEGIN
    SELECT
        ...
        p.unit_price,
        p.volume_price,
        COALESCE(SUM(i.amount_available), 0) AS stock, -- Fix COALESCE and SUM usage
        p.category,
        p.brand,
        p.fk_supplier
    FROM
        punto_de_venta.products p
    LEFT JOIN
        punto_de_venta.inventories i ON p.code = i.fk_code -- Correct join condition
    GROUP BY
        p.id, p.name, p.description, p.code, p.unit_measurement, p.unit_price,
        p.volume_price, p.category, p.brand, p.fk_supplier; -- Group by non-aggregated columns
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-11-18 15:55:04
